...
class ZephyrScaleClient extends RestClient {
    ...
    async addTestCaseIssueLink(testCaseKey, issueId) {
        if (issueId) {
            let existingLinks = await this._get(`testcases/${testCaseKey}/links/issues`);
            for (let i in issueId) {
                if (!existingLinks.some(link => link.issueId === issueId[i])) {
                    let requestBody = {
                        "issueId": issueId[i]
                    };
                    await this._post(`testcases/${testCaseKey}/links/issues`, requestBody);
                }
            }
        }
    }
